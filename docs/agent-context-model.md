...
> **🔬 待测（后面再做）**：push vs pull 的实际 token / 成本对比 —— 现在**不做**合成 A/B（贵、噪声大、且不改"混合"这个方向）。标记为待测：等真实使用数据积累后，直接用 `task_usage`（按 issue 长度 / 缓存命中率下钻）评估，成本成为痛点时再动。instrumentation 已就绪。
